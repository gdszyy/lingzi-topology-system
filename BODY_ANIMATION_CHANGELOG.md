...
1. **修复引用问题**：解决了 `BodyAnimationController` 通过 `player.visuals` 获取父节点导致的循环引用和类型转换失败问题。现在直接在初始化时传递 `CombatAnimator` 引用。
2. **增强动画幅度**：显著增大了行走、奔跑和飞行时的手臂摆动幅度，使动画效果在视觉上更加明显。
3. **优化躯干动画**：修复了躯干偏移动画未正确应用的问题，现在躯干会随呼吸和移动产生明显的上下和前后偏移。
4. **降低状态阈值**：降低了奔跑和高速飞行的触发速度阈值，使玩家更容易观察到高级动画状态。
5. **添加调试日志**：在关键状态切换处添加了日志输出，方便确认系统运行状态。
