...
const COST_BUDGET: Dictionary = {
	1: {"main": 80.0, "child": 40.0},
	2: {"main": 60.0, "child": 30.0},
	3: {"main": 45.0, "child": 22.0},
	4: {"main": 35.0, "child": 15.0}
}
...
func _generate_scenario_trigger(rules_config: Dictionary, is_first_rule: bool, scenario: SpellScenarioConfig.SpellScenario) -> TriggerData:
	...
	match trigger_type:
		...
		TriggerData.TriggerType.ON_STATUS_APPLIED:
			...
			var preferred_status = rules_config.get("preferred_status_types", [1, 3, 4])
...
func _generate_displacement_action(rules_config: Dictionary, action_budget: float) -> DisplacementActionData:
	...
	disp.displacement_type = randi() % 4
	...
	if randf() < 0.4:
		match chain.chain_type:
				ChainActionData.ChainType.LIGHTNING:
					chain.apply_status_type = ApplyStatusActionData.StatusType.CRYO_CRYSTAL
				ChainActionData.ChainType.FIRE:
					chain.apply_status_type = ApplyStatusActionData.StatusType.ENTROPY_BURN
				ChainActionData.ChainType.ICE:
					chain.apply_status_type = ApplyStatusActionData.StatusType.CRYO_CRYSTAL
				ChainActionData.ChainType.VOID:
					chain.apply_status_type = ApplyStatusActionData.StatusType.RESONANCE_MARK
...
func _generate_mine_child_spell(max_damage: float) -> SpellCoreData:
	...
	spell.carrier.velocity = 0.0
	spell.carrier.lifetime = randf_range(8.0, 15.0)
