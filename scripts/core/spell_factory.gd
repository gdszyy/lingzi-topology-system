...
var config = {
	...
	"slow_velocity_range": Vector2(30.0, 150.0),
	"lifetime_range": Vector2(1.0, 8.0),
	"mine_lifetime_range": Vector2(5.0, 15.0),
	"size_range": Vector2(0.5, 2.0),
	"mine_size_range": Vector2(1.0, 2.5),
...
func _generate_random_carrier() -> CarrierConfigData:
	...
	if type_roll < config.mine_probability:
		...
		carrier.lifetime = randf_range(config.lifetime_range.x, config.lifetime_range.y * 1.5)
